...
def build_everything(args: arg_util.Args):
    ...
    vae_local, var_wo_ddp = build_vae_var(
        ...
        class_token_scales=args.class_token_scales,
    )
    ...
    # HierAmp: initialize from a pre-trained VAR checkpoint, then fine-tune with class tokens
    if args.var_init_ckpt:
        init_sd = torch.load(args.var_init_ckpt, map_location='cpu')
        if 'trainer' in init_sd: init_sd = init_sd['trainer']['var_wo_ddp']   # allow ar-ckpt-*.pth files
        missing, unexpected = var_wo_ddp.load_state_dict(init_sd, strict=False)
        assert not unexpected, f'unexpected keys in {args.var_init_ckpt}: {unexpected}'
        bad = [k for k in missing if 'cls_tokens' not in k and 'class_head' not in k]
        assert not bad, f'missing non-HierAmp keys: {bad}'
        print(f'[HierAmp] initialized from {args.var_init_ckpt}; newly added params: {missing}')
    ...
    trainer = VARTrainer(
        ...
        var_opt=var_optim, label_smooth=args.ls, cls_lw=args.cls_lw,
    )
    ...
    if args.local_debug:
        ...
        rng.manual_seed(0)
        B = 4
        inp = torch.rand(B, 3, args.data_load_reso, args.data_load_reso, device=args.device)
        label = torch.ones(B, dtype=torch.long, device=args.device)
