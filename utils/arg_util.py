...
class Args(Tap):
    ...
    # HierAmp
    cls_scales: str = '0_1_2_3_4_5_6_7_8_9'  # scales that get a learnable class token; '' to disable (plain VAR)
    cls_lw: float = 1.0                      # weight of the class-token classification loss L_cls (Eq. 6)
    var_init_ckpt: str = ''                  # path to a pre-trained VAR checkpoint (e.g. var_d16.pth) to fine-tune from
    class_token_scales: tuple = None         # [automatically set; don't specify this] = tuple(map(int, cls_scales.split('_')))
...
def init_dist_and_get_args():
    ...
    if args.local_debug:
        ...
        args.pg = 0.8
        args.pg0 = 1
        args.cls_scales = ''   # local_debug uses 3 scales + progressive training, both incompatible with class tokens
    else:
    ...
    args.class_token_scales = tuple(map(int, args.cls_scales.replace('-', '_').split('_'))) if args.cls_scales else ()
